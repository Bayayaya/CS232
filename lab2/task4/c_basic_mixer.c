...
void conditionmixer() {
	...
	printf("!(-1) = %d\n", !(-1));	//!(-1) = 0
	printf("!\"abc\" =%d\n", !"abc");	// ! "abc" = 0
	printf("2.0f == 2.0 is %d\n", (2.0f == 2.0));	//2.0f == 2.0 is 1
	printf("1.1f == 1.1 is %d\n", (1.1f == 1.1));	//1.1f == 1.1 is 0 (b/c 1.1 is a rounding number in binary
													// thus float 1.1 doess't equal to double 1.1)
...
void typemixer() {
	...
	printf("a+j=%d\n", a+j);
	//a+j=107 (ASCII code point of "a" plus 10)
	// ---------------------------------------------------------------------
	// a character plus an integer
		
    printf("k+B=%d\n", k+B);
    //k+B=86 (20 plus ASCII code point of "B")
	// ---------------------------------------------------------------------
